...
const ScheduledMaintenance = () => {
    ...
    // Function to format asset ID in the consistent format
    const formatAssetId = (id) => {
        return id ? `AST-${id.slice(-5).toUpperCase()}` : "N/A";
    };
    ...
    const handleChange = (e) => {
        ...
        if (name === "asset") {
            ...
            if (selectedAsset) {
                setFormData({
                    ...formData,
                    assetId: selectedAsset._id, // Store the original ID for API
                    formattedAssetId: formatAssetId(selectedAsset._id), // Store the formatted ID for display
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            // Use the original assetId for the API request
            const dataToSend = {
                ...formData,
                // Make sure we're using the original asset ID for the backend
        ...
        <div className="min-h-screen bg-gradient-to-br from-[#EAD8B1] to-[#6A9AB0] flex items-center justify-center p-6 mt-20">
            ...
            >
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                            ...
                            {assetsLoading ? (
                                ...
                                assets.map((asset) => (
                                    <option key={asset._id} value={asset._id}>
                                        {formatAssetId(asset._id)} - {asset.assetName}
                                    </option>
                                ))
                            )}
                        </select>
                    ...
                    {/* Display the selected formatted asset ID */}
                    {formData.formattedAssetId && (
                        <motion.div 
                            variants={itemVariants}
                            className="bg-[#EAD8B1]/20 p-3 rounded-lg border border-[#6A9AB0]/20"
                        >
                            <p className="text-[#3A6D8C] font-medium">
                                Selected Asset: <span className="text-[#001F3F]">{formData.formattedAssetId} - {formData.assetName}</span>
                            </p>
                        </motion.div>
                    )}
