  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        setStats([
          { name: "Total Assets (Types)", value: statsData.totalAssets, color: colors.darkNavy },
          { name: "Total Quantity", value: statsData.totalQuantity || 0, color: colors.green },
      ...
      >
        <div className="p-6 bg-white">
          ...
          <div className="mt-12 grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            >
              <div className="p-4 border-b" style={{ backgroundColor: colors.accent, borderColor: colors.primary }}>
                <h3 className="text-xl font-semibold" style={{ color: colors.darkNavy }}>Quantity by Status</h3>
              </div>
              <div className="p-6 bg-white">
                <div className="w-full h-64">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={stats ? stats.filter(item => item.name !== "Total Assets (Types)" && item.name !== "Total Quantity") : []}>
                      ...
                      <Bar dataKey="value" name="Quantity" radius={[4, 4, 0, 0]}>
                        {(stats || []).filter(item => item.name !== "Total Assets (Types)" && item.name !== "Total Quantity").map((entry, index) => (
                          <Cell key={`cell-${index}`} fill={entry.color} />
                        ))}
                      </Bar>
                    </BarChart>
          ...
          >
            <div className="p-4 border-b" style={{ backgroundColor: colors.accent, borderColor: colors.primary }}>
              <h3 className="text-xl font-semibold" style={{ color: colors.darkNavy }}>Asset Category Quantity Breakdown</h3>
            </div>
            <div className="p-6 bg-white">
              <div className="w-full h-64">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={categoryStats || []}>
                    <XAxis dataKey="category" tick={{ fill: colors.darkNavy }} />
                    <YAxis tick={{ fill: colors.darkNavy }} />
                    <Tooltip 
                      contentStyle={{ 
                        borderRadius: '4px', 
                    ...
                    />
                    <Legend />
                    <Bar dataKey="count" name="Asset Types" fill={colors.darkNavy} radius={[4, 4, 0, 0]} />
                    <Bar dataKey="totalQuantity" name="Total Quantity" fill={colors.green} radius={[4, 4, 0, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </div>
